...
- 紧凑面板保留在当前右下区域，并锚定到原生底部装备带的上方。它使用与 CS2 原生脚本相同的缩放布局尺寸读取方式检测 Panorama 画布，在窄屏模式下抵消横向拉伸，同时跟随回放栏模式保留垂直间距；外观保留原生 HUD 的模糊、阴影和控制条尺寸，但使用高不透明度阅读底以保证语音与玩家信息清晰，标题栏悬停时也始终保持深色。点击整个标题栏区域可展开或收起。
