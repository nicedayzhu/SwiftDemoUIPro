...
- 为高阶玩家提供轻量的高级启动项入口，点击后在独立弹框中设置。自定义参数只保存在本机并附加到启动器发起的会话，不能覆盖由启动器管理的安全参数。
...
- 点击存活玩家切换第一人称 POV；当前 POV 会以金色高亮并标记为**当前**。勾选列表上方的**显示头像**后，每行会同时显示 Steam 头像、玩家名称和阵营/槽位信息，便于快速找人。
...
- 玩家选中状态遵循原生阵营配色：T 玩家使用金黄/浅黄色强调，CT 玩家使用蓝色强调，并同时作用于当前观察行与已启用语音状态。
- 左下角说话状态由当前 Demo tick 驱动，因此跳转、暂停和倍速播放后都能重新计算，不依赖实时麦克风回调。
- 展开**回合导航**，选择回合即可跳转到其起始 tick。
- 面板默认位于右下区域、原生底部装备带的上方。在 Demo 鼠标模式下，可拖动标题栏左侧的金色喇叭移动面板；鼠标会自动变为移动形状，原菜单从旧位置消失，并由跟随鼠标的半透明真实尺寸放置轮廓表示最终占用范围。预览靠近屏幕边缘时会自动吸附，最终位置也会被限制在屏幕内。点击恢复图标可回到响应式默认位置，点击标题栏其余区域可展开或收起，收起前后保持足够的文字宽度。位置在当前 Panorama 会话中保留，并会在画布变化时重新约束。
